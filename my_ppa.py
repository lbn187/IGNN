...
@torch.no_grad()
def test(model, data, predictor, new_x, split_edge, evaluator, subgraph_loader, batch_size, device):
    predictor.eval()
    model.eval()
    #data = data.to('cpu')
    #model = model.to('cpu')
...
def main():
    ...
    #data.x = torch.cat([data.x, new_x], dim=1)
    ...
    for run in range(args.runs):
        ...
        if args.use_save:
            model.load_state_dict(torch.load('./save_model_0123'))
            predictor.load_state_dict(torch.load('./save_predictor_0123'))
            new_x.load_state_dict(torch.load('./save_x_0123'))
        ...
        for epoch in range(1, 1 + args.epochs):
            ...
            print(loss)
            torch.save(model.state_dict(), './save_model_0123')
            torch.save(predictor.state_dict(), './save_predictor_0123')
            torch.save(new_x.state_dict(), './save_x_0123')
            if epoch > args.starteval_epoch and epoch % args.eval_steps == 0:
