...
def train(model, predictor, x, adj_t, train_pos_edge, train_pos_edge_info, mat, optimizer, batch_size, device):
    row, col, _ = adj_t.coo()
    ...
    for perm in DataLoader(range(train_pos_edge.size(0)), batch_size, shuffle=True):
        ...
        edge_info = torch.FloatTensor(pos_out.size(0), 1)
        for i in range(pos_out.size(0)):
            edge_info[i, 0] = mat[edge[0, i].item(),edge[1, i].item()].item()
        neg_out = predictor(h[edge[0]], h[edge[1]], edge_info.to(device))
...
def test(model, predictor, x, adj_t, valid_pos_edge, valid_pos_edge_info, valid_neg_edge, valid_neg_edge_info, test_pos_edge, test_pos_edge_info, test_neg_edge, test_neg_edge_info, evaluator, batch_size, device):
    ...
    for perm in DataLoader(range(valid_pos_edge.size(0)), batch_size):
        ...
        #pos_out = 0.9 * pos_out + 0.1 * torch.sigmoid(-84.0 * edge_info + 28.8)
    ...
    for perm in DataLoader(range(valid_neg_edge.size(0)), batch_size):
        ...
        #neg_out = 0.9 * neg_out + 0.1 * torch.sigmoid(-84.0 * edge_info + 28.8)
...
def main():
    ...
    f = open("ddineg/train_pos_avg.txt","r")
    lines = f.readlines()
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            loss = train(model, predictor, emb.weight, adj_t, train_pos_edge, train_pos_edge_info, mat, optimizer, args.batch_size, device)
            #print(loss)
            print(loss)
            torch.save(model.state_dict(), './ddi_model')
            torch.save(predictor.state_dict(), './ddi_predictor')
            torch.save(emb.state_dict(),'./ddi_x')
            if epoch > args.starteval_epoch:
                ...
                for key, result in results.items():
                    ...
                    if valid_hits > 0.0 and key == "Hits@20":
                        print(key)
