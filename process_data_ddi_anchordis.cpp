...
int main(){
	...
	for(int i=0;i<K;i++){
		...
		for(int j=0;j<train_pos_m;j++){
			train_pos[j].z+=dd[train_pos[j].x]+dd[train_pos[j].y];
		}
		for(int j=0;j<train_neg_m;j++){
			train_neg[j].z+=dd[train_neg[j].x]+dd[train_neg[j].y];
		}
		for(int j=0;j<valid_pos_m;j++){
			valid_pos[j].z+=dd[valid_pos[j].x]+dd[valid_pos[j].y];
		}
		for(int j=0;j<valid_neg_m;j++){
			valid_neg[j].z+=dd[valid_neg[j].x]+dd[valid_neg[j].y];
		}
		for(int j=0;j<test_pos_m;j++){
			test_pos[j].z+=dd[test_pos[j].x]+dd[test_pos[j].y];
		}
		for(int j=0;j<test_neg_m;j++){
			test_neg[j].z+=dd[test_neg[j].x]+dd[test_neg[j].y];
		}
	}
	freopen("ddi/train_pos_anchordis.txt","w",stdout);
	for(int i=0;i<train_pos_m;i++)printf("%d\n",train_pos[i].z);
	fclose(stdout);
	freopen("ddi/train_neg_anchordis.txt","w",stdout);
	for(int i=0;i<train_neg_m;i++)printf("%d\n",train_neg[i].z);
	fclose(stdout);
	freopen("ddi/valid_pos_anchordis.txt","w",stdout);
	for(int i=0;i<valid_pos_m;i++)printf("%d\n",valid_pos[i].z);
	fclose(stdout);
	freopen("ddi/valid_neg_anchordis.txt","w",stdout);
	for(int i=0;i<valid_neg_m;i++)printf("%d\n",valid_neg[i].z);
	fclose(stdout);
	freopen("ddi/test_pos_anchordis.txt","w",stdout);
	for(int i=0;i<test_pos_m;i++)printf("%d\n",test_pos[i].z);
	fclose(stdout);
	freopen("ddi/test_neg_anchordis.txt","w",stdout);
