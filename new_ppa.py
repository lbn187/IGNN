...
class LinkPredictor(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim, num_layers, dropout, device):
        ...
        #self.concat_layers.append(torch.nn.Linear(2,2))
...
def train(model, predictor, emb_x, datax, data, train_pos_edge, train_pos_edge_info, train_neg_edge, train_neg_edge_info, optimizer, batch_size, device):
    #row, col, _ = adj_t.coo()
    #edge_index = torch.stack([col,row], dim = 0)
    model.train()
    predictor.train()
    x = torch.cat([emb_x, datax], dim=1).to(device)
    ...
    for perm in DataLoader(range(train_pos_edge.size(0)), batch_size, shuffle=True):
        ...
        loss.backward()
        torch.nn.utils.clip_grad_norm_(emb_x, 1.0)
...
@torch.no_grad()
def test(model, predictor, emb_x, datax, data, valid_pos_edge, valid_pos_edge_info, valid_neg_edge, valid_neg_edge_info, test_pos_edge, test_pos_edge_info, test_neg_edge, test_neg_edge_info, evaluator, batch_size, device):
    #predictor.eval()
    model.eval()
    predictor.eval()
    # emb_x.eval()
    x = torch.cat([emb_x, datax], dim=1)
    h = model(x, data.adj_t)
    #model.to('cpu')
...
def main():
    ...
    f = open("save/train_pos_avg.txt","r")
    lines = f.readlines()
    ...
    if args.model_name == "GCN":
        model = GCN(data.num_features + args.in_newx, args.hidden_channels, args.hidden_channels, args.num_layers, args.use_res, args.dropout, device).to(device)
        ...
        model = MyModel(data.num_features + args.in_newx, args.hidden_channels, args.hidden_channels, args.num_layers, args.use_res, args.dropout, device).to(device)
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            loss = train(model, predictor, emb.weight, datax, data, train_pos_edge, train_pos_edge_info, train_neg_edge, train_neg_edge_info, optimizer, args.batch_size, device)
            if epoch > args.starteval_epoch:
                valid_loss, test_loss, results = test(model, predictor, emb.weight, datax, data, valid_pos_edge, valid_pos_edge_info, valid_neg_edge, valid_neg_edge_info, test_pos_edge, test_pos_edge_info, test_neg_edge, test_neg_edge_info, evaluator, 10000, device)
                for key, result in results.items():
                    ...
                    if test_hits > max_test_hits and key == "Hits@100":
